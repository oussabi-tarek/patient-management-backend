...
const getPatientInfo = asyncHandler(async (req, res) => {
  const foundPatient = await Patient.findOne({ email: req.body.email }).select(
    "-password"
  );
...
const updateImageProfile = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Internal Server Error during upload imagee" });
  }
});

const deleteImageProfile = asyncHandler(async (req, res) => {
  const patientId = req.body._id;
